---
title: "Additional Resources"
type: "wiki"
weight: 2000
---

## List of additional resources for the plugins/tools we use.
- [WorldEdit Documentation](https://worldedit.enginehub.org/en/latest/)
- [WorldEdit Brushes](https://worldedit.enginehub.org/en/latest/usage/tools/brushes/)
- [WorldEdit Patterns](https://worldedit.enginehub.org/en/latest/usage/general/patterns/)
- [WorldGuard Documentation](https://worldguard.enginehub.org/en/latest/)
- [WorldGuard Flags](https://worldguard.enginehub.org/en/latest/regions/flags/)
- [JavaScript docs (For Scripting)](https://developer.mozilla.org/en-US/docs/Web/JavaScript)
- [Block Display Studio](https://eszesbalint.github.io/bdstudio/editor)
- [McStacker (For making commands)](https://mcstacker.net/)
- [BlockBench (For Texture Pack)](https://www.blockbench.net/)
- [minecraft.tools](https://minecraft.tools/en/)
- [Minecraft Wiki](https://minecraft.wiki/)